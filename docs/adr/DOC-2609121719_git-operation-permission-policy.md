...
1. **ヒアドキュメントの本文をトークン化の前に落とす**（`<<WORD` / `<<'WORD'` /
   ...
   **delimiter の文字種はシェルが受け付ける範囲（空白・リダイレクト記号・
   クォート以外）まで許し、終端判定は `<<` と `<<-` を区別する**（`<<-` のときだけ
   先頭の空白を剥がして比較する）。`EOF` だけに絞ると `<<'EOF-1'` の本文が
   剥がれず、区別せず `strip()` で比較すると本文中のインデントされた終端語
   （ヒアドキュメントの例を含む文章）で早々に切れて、以降の本文が素のコマンド
   扱いになる。どちらも上と同じ「ただのファイル書き込みが `deny`」に倒れる
...
3. **`punctuation_chars=True` の lexer を使う。** `;` は前の語に空白が無ければ
   ...
   明示的に空にする。** 有効なままだと行内の裸の `#`（URL のフラグメント等）
   以降が丸ごと捨てられ、`curl https://x/y#z && gh pr merge 95` を取りこぼす
