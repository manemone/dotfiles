...
│   │   ├── .dotfiles-manifest
│   │   ├── shared/
│   │   └── zsh/ nvim/ tmux/ bin/ claude/ skills/ codex/ opencode/
│   ├── 20260804T101200-25e1430/
│   └── 20260804T113000-fab7694/
├── settings.machine.json              # Claude Code のマシン固有設定の実体(世代を経由しない。
│                                       # ~/.claude/settings.machine.json はここへの symlink。
│                                       # 計画書 DOC-2609121700 設計6。claude/README.md §4参照)
├── CLAUDE.machine.md                  # 人格のパーソナライズ設定の実体(世代を経由しない。
│                                       # ~/.claude/CLAUDE.machine.md はここへの symlink。
│                                       # 人間の持ち物として保護される。ADR DOC-2609162327 §2)
├── codex/
│   └── AGENTS.md                      # claude/CLAUDE.md + CLAUDE.machine.md を連結生成した
│                                       # Codex 向け実ファイル(世代を経由しない。~/.codex/AGENTS.md
│                                       # はここへの symlink。常に再生成可能な生成物であり保護
│                                       # されない。ADR DOC-2609162327 §6)
...
- `current` は `$HOME` 側の全 symlink が経由する唯一の間接参照点です。`deploy-all.sh` および
  ...
  備えて `skill_links()` 等は世代ディレクトリを直接指すリンクも認識対象にしています。
  後述の「旧方式からの移行手順」も参照）。**例外は `~/.claude/settings.machine.json` /
  `~/.claude/CLAUDE.machine.md` / `~/.codex/AGENTS.md` の3つです。** これらは `current`
  ではなく、世代を経由しない固定パス（前掲のディレクトリ構造の `settings.machine.json` /
  `CLAUDE.machine.md` / `codex/AGENTS.md`）を直接指す symlink であり、`current` の付け替えの
  影響を受けません
- 世代ディレクトリ名は `<date +%Y%m%dT%H%M%S>-<short sha>`。同一秒・同一コミットで
  再デプロイすると ID が衝突し、上書きせずエラーになります（`shared/helpers.sh` の
  `create_generation`）
- 世代の中身は常に `AVAILABLE_TOOLS`（`zsh nvim tmux bin claude skills codex opencode`）
  全部 + `shared/`。`--only` はこの中身に影響しません（`$HOME` のどのリンクを張るかだけを
  制御します）
...
- `$HOME` 側の全 symlink（`skills/` 配下の個別 symlink を、全エージェント分含む）の健全性。
...
`$HOME` 側の symlink は張り直されません。ほとんどのリンクは `current` という1本のシンボリック
リンク経由で解決される設計のため、`current` の付け替え1回だけで向き先が
一斉に切り替わります（ADR DOC-2608040229 §4.1）。

**ロールバックが戻さないもの**:

- `~/.claude/settings.json` は生成された実ファイルであり symlink ではないため、`current` の
  付け替えの影響を受けません。ロールバック後にその内容も戻したい場合は、ロールバック先の
  世代から `claude/deploy.sh` を明示的に再実行してください
- `~/.codex/AGENTS.md` は symlink ですが、`current` を経由せず世代の外の固定パス
  （`<prefix>/codex/AGENTS.md`。前掲のディレクトリ構造を参照）を直接指しているため、
  `current` の付け替えでは内容が変わりません。ロールバック前の `claude/CLAUDE.md` +
  `CLAUDE.machine.md` の内容のまま残るため、追随させたい場合は `persona --regen`
  （または対象世代からの `codex/deploy.sh` 再実行）を明示的に実行してください
  （ADR DOC-2609162327 §6.2）
- `~/.claude/settings.machine.json` / `~/.claude/CLAUDE.machine.md` は人間が書き込む
  マシンローカルな実体で、そもそも `current` を経由しないため、ロールバックの影響を
  受けません（元から変化しない、という意味で「戻す/戻さない」の対象外です）
...
- `uninstall.sh` は dev モード中でも `generations/` `.tmp/` と `current` 自体は通常どおり
  片付けます。保護されるのは **`current` が指す作業ツリーの実体**と、前節の
  `settings.machine.json` / `CLAUDE.machine.md`（作成済みなら）で、そちらには一切触れません。
  一方、`<prefix>/codex/AGENTS.md`（Codex 向け連結生成物）は人間の持ち物ではなく常に
  再生成可能な生成物のため保護対象ではなく、codex がスコープに含まれる撤去では削除されます
...
- `skills/` 配下の個別 symlink も同様の判定です。旧方式で張られたスキルの symlink は、
  今回 deploy を実行しているチェックアウトを指している場合に限り
  `<エージェントのホーム>/skills-backup/` へ退避されず張り替えられます。
  スキルが `claude/skills/` から `skills/` へ切り出される前（ADR DOC-2608272128）に
  張られた symlink も、リポジトリ由来として同じ扱いになります
