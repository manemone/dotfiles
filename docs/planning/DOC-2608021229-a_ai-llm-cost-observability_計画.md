...
| 5 | `ai/ph-05-reports-baseline` | 集計レポート（PR別/工程別/model別/5h窓別/月次）+ ベースライン計測手順 + 文書索引・スキーマ仕様 | ✅ PR #32 マージ済 |
...
### 傘マージ後の残タスク（本傘のスコープ外・費用最適化とは別系統）
...
> `ADR-001` → `DOC-2608021229` に移行し、全参照を追従した。以下は移行前に書かれた記述であり、
> 歴史的記録として残す。

現行の連番方式（`DOC-001` / `DOC-002` / `DOC-003`）は早晩崩壊する。

- **並行ブランチで衝突する** — 複数の孫が同時に文書を作れば全員が同じ次番号を取る。
  本傘は「文書を作る孫が孫5だけ」だったため偶然回避されているにすぎない
...
| 参照元 | 影響 |
|---|---|
| 計画書 `DOC-003` の自己参照（孫プロンプト内）13箇所 | 孫が計画書を見失う |
| 稼働中の autopilot cron プロンプト | **計画書の絶対パスが直書き。cron は作成後に編集できない** |
| 実行中の implementer セッションに送った起動プロンプト | 参照中 |
| `bin/tests/lint.sh` のコメント | 軽微 |

→ **傘が `master` にマージされ、autopilot を停止した後に、独立したPRで一括移行する。**

移行PRでやること:

1. 既存文書のリネーム（`git mv` で履歴を保つ）。タイムスタンプは `git log --diff-filter=A`
   で得た**初出コミット日時**から起こす（`DOC-001` / `DOC-002` / `DOC-003` / `ADR-001` /
   本傘で追加する `DOC-004` / `DOC-005`）
2. 全参照の追従（`grep -rl` で機械的に洗い、リンク切れゼロを確認）
3. `docs/README.md` の索引更新
4. **採番規約を `docs/README.md` に明記** — 「新規文書は作成時刻から `DOC-YYMMDDHHMM`。
   連番は使わない」。規約が書かれていなければ次に触る人がまた連番を振る
5. ID重複・リンク切れの検証コマンド（lora-dataset-forge の `bin/doc-id verify` 相当）を
   入れるかは別途判断。入れるなら `bin/tests/lint.sh` から呼ぶ

**この残タスクを忘れないこと。** 孫5が作る `docs/README.md` にも、
現行が連番であることと本移行が予定されていることを but書きとして残す（孫5用プロンプト #3.5 参照）。
...
`~/projects/lora-dataset-forge/main/docs/` が同一ユーザーの確立した文書規約である。
**そちらの `docs/README.md` を実際に開いて構成を確認してから着手すること。** 要点:
...
**リポジトリ内の一貫性を優先し、連番を継続する**（既存の最大番号を確認し次の空き番号を使う）。
採番方式を変えるならそれは独立した判断であり、このPRでやらない。
...
一方で保存済みイベントは `schema_version: 1` を持ったまま残り続け、将来の集計コードが読む。
**44,000件規模のデータの意味が、完了済み計画書の実装者プロンプトの中にしか無い状態を解消する。**

必須内容:

1. **共通エンベロープの全フィールド表** — フィールド名 / 型 / 必須任意 / 意味 / 実例
   - 特に `completeness`（`complete` / `partial` / `unknown`）の判定基準
   - `source` と `parser_version` の組み合わせが何を意味するか
   - `null` と `0` の区別（`reasoning_tokens: null` は「取れていない」であって「ゼロ」ではない）
2. **全 `event_type` の一覧とペイロード** — `run.start` / `run.end` / `phase.start` / `phase.end` /
   `pr.bind` / `review.round` / `human.intervention` / `usage.message` / `quota.sample` /
   `block.start` / `block.end` / `meter.error`。**実装されていない型は「未実装」と明記する**
3. **`idempotency_key` の生成規則**（型ごと）と重複排除の意味論
4. **費用計算式**と `price_table_version` / `price_effective_date` の役割
   （**過去データを新価格で再計算しない**という不変条件を明記）
5. **`window_id`（5時間枠）の扱い** — 鮮度判定、跨ぎで減算しない理由
6. **`schema_version` の変更ルール** — 何をしたらv2になるか、v1データをどう読み続けるか
   （フィールド追加は互換 / 意味の変更・削除は非互換、等の方針を決めて書く）
7. **実イベントの実例**（秘密情報を含まないもの）を型ごとに1件ずつ貼る

`bin/README.md` からこの文書へリンクし、**計画書 `DOC-003` への参照はスキーマの情報源としては使わない**
（設計の経緯を知りたい人向けの副次的な参照に留める）。

#### 3.5. `docs/README.md`（文書索引）の新設

本リポジトリの `docs/` には索引が無く、どこに何があるか分からない。
...
既存の `DOC-001` / `DOC-002` / `DOC-003` / `ADR-001` もすべて索引に載せること。
**中身を読まずにタイトルだけで概要を書かない。**

さらに、**採番方式について次の but 書きを必ず残すこと**（第16章「傘マージ後の残タスク T1」と対応）:

> 現在の DOC-ID は連番（`DOC-001` …）だが、この方式は並行ブランチで採番が衝突するため、
> タイムスタンプ式（`DOC-YYMMDDHHMM`、`~/projects/lora-dataset-forge` と同方式）への
> 移行を予定している。移行は独立したPRで行う。**それまでは連番を継続すること。**

これが無いと、次に文書を追加する人が同じ議論を再演するか、無自覚に連番を振り続けることになる。

#### 4. 保持期間の扱いを決める（積み残しの解消）

計画書9.3は「保存期間: 既定14日、`ocw-meter prune` で削除」と書いているが、
**`prune` はどの孫のスコープにも入っていない**（計画時の書き漏れ）。次のいずれかで解消する:

- **A. 実装する**: `ocw-meter prune [--older-than <days>] [--dry-run]` を追加。
  既定は raw のみ削除し、**`events/` は既定では消さない**（費用履歴は資産であり、
  数十MB/年でしかない）。`--include-events` を明示したときのみイベントも対象にする
- **B. 実装しない**: `bin/README.md` に「**保持期間の自動削除は未実装。
  不要になったら手動で削除すること**」と明記し、削除して安全なパスを列挙する

どちらでもよいが、**「計画書には書いてあるが実装が無い」状態のまま残さないこと。**
判断理由をPR説明文に書くこと。
