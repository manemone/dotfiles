...
**配置の注意**: `docs/` 直下にファイルを置く慣習は無い（現状0ファイル）。
目的別サブディレクトリに分ける慣習である（`docs/adr/` = 決定記録、`docs/planning/` = 計画書）。
運用し続けるリファレンス／手順書は **`docs/reference/`（新規ディレクトリ）** に置くこと。
`DOC-00N` は文書全体の連番。着手時に既存の最大番号を確認し、埋まっていれば次の空き番号を使う。
...
`docs/reference/**`（新規ディレクトリ。DOC-004 手順書 / DOC-005 スキーマ）,
...
- 新規文書が `docs/reference/` に置かれ、`docs/` 直下にファイルを増やしていない
