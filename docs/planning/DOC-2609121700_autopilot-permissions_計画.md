...
## マージ後の実測: 出てしまった回帰（2026-09-13）

**傘を `master` へマージし deploy した直後の実環境で、ガードフックが無関係な日常コマンドを
巻き添えで止める回帰が出た。** 傘の目的（承認ダイアログを減らす）に対して差し引き
マイナスの状態になった。人間の評価（逐語）:

> こういうのがでるんだが

> 今まで出たことない

> 新しい問題が生まれてるじゃねえか、、、、めんど
...
1. **`mentions_guarded_command()` が「`git` という単語」で判定している**
   （`claude/hooks/git-guard.sh`。`re.search(r"\bgit\b", command)`）。これが
   `has_chain()` と AND されるため、**`git` を含む連結コマンドは読み取り専用でも全部 `ask`**
   になる。`cd <repo> && git status` が止まる
2. **`subcommand_of()` が `git` の直後のトークンが `-` 始まりなら uncertain を返す**ため、
   **`git -C <dir> <sub>` が常に `ask`** になる
3. この2つの合成で、**別ディレクトリで git を叩く手段が全滅する**
   （`cd && git` は1、`git -C` は2、素の `git` は cwd が固定される）
4. **`has_chain()` は `&&` だけでなく `|` も連結として扱う**ため、
   `git log | head` `git status | grep ...` のようなパイプ付きの読み取り専用
   コマンドも `ask` になる。エージェントが Bash を叩くときはパイプか `&&` が
   ほぼ必ず付くため、**実運用では「git と名のつくものはほぼ全部ダイアログ」**になる。
   素の `git add -A` のような単体コマンドは通るが、それは実際の使い方では例外的である
5. トークン化に失敗した任意の git コマンド（クォートが絡む形）も同じ経路で `ask` になる

### 受け入れ条件の欠陥（司令官の責任）

孫1・孫5 の「検証方針」と `tests/git_guard_test.sh` は、**判定表どおりに deny/ask/allow
できるか**を厚く検証していた。しかし**ガード対象でないコマンドが巻き添えで `ask` になる率**
を誰も測っていない。孫1 の検証方針には
「git / gh と無関係なコマンドでフックが何も出力しないこと」という項目はあったが、
これは**「git を含むが無害なコマンド」（読み取り専用 git・連結・`git -C`）を
カバーしていなかった**。最も踏みやすい範囲がちょうど抜けていた。

`allow` に倒さない（fail-safe）を優先した判断自体は正しい。問題は、**その巻き添えコストが
傘の目的そのものを打ち消す規模だったのに、受け入れ条件にその観点が無かった**こと。

**今後この種のガードを足すときは、判定表と同じ重みで「無関係な日常コマンドが何 % 止まるか」を
受け入れ条件に入れること。** 具体的には、実際の作業ログから採った通常コマンド列
（`git status` / `git log` / `cd X && git ...` / `git -C ... ...` / `gh pr view` 等）を
フックに流し、**1件も `ask` にならないこと**を自動テストで固定する。

### 対応

この回帰の修正は**この傘の外で、単発 PR として別途行う**（方針: `mentions_guarded_command()`
をガード対象の操作パターンで判定する／`subcommand_of()` が値を取るグローバルオプション
`-C <path>` `-c <k=v>` `--git-dir=` `--work-tree=` を読み飛ばす／回帰テストを
`tests/git_guard_test.sh` に追加）。傘は既にマージ済みであり、差し戻さない。
