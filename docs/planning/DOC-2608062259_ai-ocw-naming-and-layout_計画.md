...
| 4 | `ai/ocw-04-docs` | 文書（`bin/README.md` §3.1 の全面改訂・ルート `README.md` の追随） | ✅ PR #53 マージ済 |
...
Herdr のワークスペースラベル（`${repo_name} :: ${slug}`。区切りを `/` から `::` に変更した。
`slug` 自体がスラッシュを含みうるため、`/` のままでは境界が視覚的に区別できないため）に
何を使うかも確認すること。
...
- **正規化で救えるものは救われる**（`die` しない）: `feature//foo` → `feature/foo` /
  `HEAD` → `head` / `-x` → `x`
- **正規化で救えないものが `check-ref-format` に弾かれて `die` する**: `foo.lock` / `a.lock/b` /
  `..` / `a/../b`
- 正規化後に空になる入力で `die` する
- `..` を含む入力が `check-ref-format` に弾かれ、リポジトリ外にディレクトリが作られない

> **訂正（PR #51 のレビューで発覚した計画書側の誤り）**: 当初この節には
> 「不正なブランチ名で `die` する: `feature//foo` / `foo.lock` / `HEAD` / 先頭ハイフン」と
> 書いていたが、これは誤りだった。ADR DOC-2608062258 §2.6 の表は
> **`git check-ref-format --branch` に「生の入力」を渡したときの実測結果**であり、
> ocw の利用者から見た挙動を規定したものではない。実際には正規化（ADR §3.2 で「残す」と
> 決めたもの）が先に走るため、`feature//foo` は `feature/foo` に、`HEAD` は `head` に、
> `-x` は `x` に変換されて検証に到達する。
>
> **正規化優先が正しい。** `ocw "Fix The Thing"` → `fix-the-thing` を許す機構と同一のもので
> あり、そこだけ弾くのは一貫性を欠く。`check-ref-format` は「正規化で中和できなかったもの」
> （`.lock` 末尾・`..`・先頭 `/`）を捕まえる**バックストップ**として機能する。
> **パストラバーサル防御は健在である**ことを実測で確認済み（`..` も `a/../b` も `die` する）。
...
- **入力が「正規化 → 検証」の順で処理されること**と、その帰結。正規化で救えるもの
  （`feature//foo` → `feature/foo`、`HEAD` → `head`、`-x` → `x`、`Fix The Thing` →
  `fix-the-thing`）は**エラーにならず変換されて通る**。`check-ref-format` が弾くのは正規化で
  中和できなかったもの（`foo.lock` の `.lock` 末尾、`..`、先頭 `/`）だけである。
  **「不正な名前は全部エラーになる」と書かないこと**（PR #51 のレビューで、計画書側が
  この誤解をしていたことが判明した。詳細は孫2用プロンプトの「テスト」節の訂正を参照）
...
- 傘ブランチ上で以下がすべて通る:
  ...
  - `bin/tests/lint.sh`
- **無設定（既定）での挙動が、次の点を除いて現行と同一である**（実装時の実測により、
  当初の想定3点に加えて以下の4点があることが判明した。詳細は ADR §5）
  1. ブランチ名に `ai/` が付かない
  2. squash マージ済みブランチが `-f` なしで削除できる
  3. `run.end` の `outcome` が `-f` の有無ではなくマージ判定から決まる
  4. `repo_name` の解決順に `remote.origin.url` からの導出が優先で入るため、表示名が変わりうる
  5. Herdr ワークスペースラベルの区切りが `${repo_name} / ${slug}` から `${repo_name} :: ${slug}` に変わる
  6. `ocw rm` はワークツリーのディレクトリが既に手動で消されている場合、`die` ではなく warn して続行する
  7. `ocw rm` のブランチ削除が `git branch -d` ではなく `-D` になり、失敗しても `die` せず warn する
