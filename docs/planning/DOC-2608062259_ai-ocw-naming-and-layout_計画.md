...
| 孫 | ブランチ | 内容 | 状況 |
|---|---|---|---|
| 1 | `ai/ocw-01-config-context` | 設定リーダ（git config `ocw.*`）と `init_repo_context()` の再構築（bare 対応・パス雛形の展開と検証・掃除境界の逆算・`repo_name` の解決） | 🔄 実装中 |
| 2 | `ai/ocw-02-naming` | 命名（`ai/` 接頭辞の廃止・ブランチ名／ディレクトリ名のサニタイズ分離・`git check-ref-format --branch` による検証・ネストディレクトリでの作成） | ⬜ 待機中 |
