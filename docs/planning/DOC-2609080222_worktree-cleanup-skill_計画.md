...
1. **マージ判定の食い違いの解消**（`skills/umbrella-orchestrator`）— 実測では7本のうち6本
   （孫5本＋傘1本）が `ocw rm -f` を要求したのに、既存スキルは「`--force` は基本的に不要」と
   書いている。原因を突き止め、記述を実態に合わせる
...
| 2 | `worktree-cleanup-02-cleanup-skill` | 掃除スキル `skills/worktree-cleanup/` の新設と `skills/README.md` の追随 | ✅ PR #79 マージ済 |
...
**実測はこれに反した（背景4参照）。** 掃除した7本のうち6本（孫5本＋傘1本）が `-f` を要求した。
同スキルはこの拒否について「傘運用で実際に遭遇するのはほぼこちら」とも書いており、**記述内部
でも整合していない**。
...
- 拒否メッセージは2種類あり、原因も対処も異なる:
  - `branch is not merged into any known integration ref: <branch>` — 候補 ref は解決できたが、
    is-ancestor でも squash 検出でも「マージ済み」と判定できなかった場合。**実測7本中6本の
    拒否はすべてこちら**
...
- **実測では、傘へ squash マージ済みの孫5本すべてが `-f` を要求した（傘1本も `-f` を
  要求したが、別原因のため孫の判定とは区別して扱う。詳細は背景4.1）。** 拒否メッセージは
...
- **実測との食い違い**: 孫5本すべて（＋別原因の傘1本）が `-f` を要求した事実を、実測値
  として記述に反映する（何本中何本か、どの拒否メッセージだったかまで書く。このスキルの
  既存の書きぶりは実測を数値付きで残す作法になっているので、それに揃える）
