...
| 孫 | ブランチ | 内容 | 状況 |
|---|---|---|---|
| 1 | `file-handoff-01-dfup` | 共通基盤（`bin/dfxfer-lib.sh`）＋上りコマンド `dfup`。配布登録・lint登録・README・テスト一式 | ✅ PR #102 マージ済 |
| 2 | `file-handoff-02-dfdown` | 下りコマンド `dfdown`。孫1の共通基盤に乗せる。ホストごとの受信ディレクトリ | ✅ PR #103 マージ済 |
