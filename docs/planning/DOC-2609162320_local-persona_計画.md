# 計画書: 配布される口調設定をデプロイ先でローカルにカスタマイズ可能にする
...
`claude/CLAUDE.md`（配布物）の冒頭に口調設定が直書きされており、このファイルは symlink で
Claude Code / Codex / OpenCode の3エージェントへ配られている（ADR DOC-2609072334）。
symlink である以上、デプロイ先で口調をいじる＝リポジトリの追跡ファイルを書き換えることになり、
**マシン固有の口調を持つ手段が一切ない**。`settings.json` には `settings.machine.json` という
マシンローカルな上書きの枠組みがあるのに、`CLAUDE.md` には無い。

本傘は、口調を**世代を経由しない固定パスのマシンローカルファイル**
...
専用の `@` import 行が入るため、**OpenCode / Codex には import 行が文字列のまま届き、口調は
効かない**（デフォルト＝口調指定なしの状態になる）。傘は `master` へまとめて入るので実害は無い。
...
- **口調は Claude Code / Codex / OpenCode の3エージェント全部で効かせる。**
  Claude Code だけに絞る案は明示的に却下された
- **デフォルト（ローカル設定が空）のとき、ベース側にデフォルト文言を書いておく。**
  「口調の節ごと出さない」案は却下された。何も指定していない状態が明示されるほうを選んだ
- **Codex 向けには専用の再生成コマンドを用意する。** Codex だけ口調非対応にして
  割り切る案・`codex/` への配布自体をやめる案は、どちらも却下された
- **OpenCode 向けに `opencode.json` を新規配布物にしてよい。** OpenCode も口調
...
`claude/CLAUDE.md` の冒頭に口調設定が直書きされている（2026-09-16 時点の現物）:
...
`codex/deploy.sh` のコメントが、口調がまさにこの共有対象であることを明言している:
...
symlink である以上、デプロイ先で口調をいじる＝**リポジトリの追跡ファイルを書き換える**
ことになる。マシン固有の口調を持つ手段が一切ない。`settings.machine.json` に相当する
...
このマシンの口調・人格設定は次の取り込みファイルが定める。
空であれば口調の指定は無し（通常どおりの口調で応答する）。
...
必要に応じて調整してよいが、**「定義元を指す」構造と「空なら口調指定なし」のデフォルトは崩さない。**

**人間の現在の口調の移行**: deploy は空の `CLAUDE.machine.md` を作るだけで、現行の
「脳筋後輩」文言を自動で書き込まない（デフォルトは口調指定なし、が決定事項）。
...
`~/.codex/AGENTS.md` から symlink で参照させる。口調変更時はコマンド1発で再生成する。
...
も含めた「口調を変えるときの入口」が1つになる。名前は孫3 が決める（`bin/` の既存メンバーは
...
- **`claude/CLAUDE.md` の口調以外の内容**（「傘ブランチへの引き継ぎ判断」の節）には触らない
...
- 人間のマシンでの実 deploy と、現行の口調文言の `CLAUDE.machine.md` への書き込み
...
3. 傘→`master` の PR に、**人間が deploy 後に行う移行手順**（現行の口調文言を
...
（背景1〜5、設計1・2・5 を必ず読むこと）に基づいて実装してください。

## ゴール

人間が `~/.claude/CLAUDE.machine.md`（実体は `<prefix>/CLAUDE.machine.md`）を書き換えるだけで、
deploy なしに Claude Code の口調が変わる状態にする。ベースの `claude/CLAUDE.md` は
「定義元を指す」デフォルト文言だけを持つ。
...
7. `claude/README.md` とルート README、ルート `AGENTS.md`「claude の例外」節を追随させる。
   README には「口調は `~/.claude/CLAUDE.machine.md` を直接編集する。deploy 不要」と、
   **現行の口調文言を移行する手順**を書く
...
- **`CLAUDE.machine.md` を持たない状態・持たないワークツリーから deploy しても、固定パスにある
  既存の実体（人間の口調設定）が失われない・上書きされないこと。**
...
- `bin/` に新コマンドを足さない（孫3）
- 現行の口調文言を deploy で `CLAUDE.machine.md` へ自動で書き込まない（デフォルトは口調指定なし）
...
あなたはこの孫ブランチの実装 AI です。計画書
`docs/planning/DOC-2609162320_local-persona_計画.md`
（背景2・4・5、設計3・5 を必ず読むこと）に基づいて実装してください。
...
人間が `CLAUDE.machine.md` を書き換えるだけで、deploy なしに OpenCode の口調も変わる状態にする。
...
````
# 孫3: Codex 向けの連結生成と口調編集コマンド

あなたはこの孫ブランチの実装 AI です。計画書
`docs/planning/DOC-2609162320_local-persona_計画.md`
（背景2・4・5、設計4・5 を必ず読むこと）に基づいて実装してください。
...
連結した実ファイルを生成して `~/.codex/AGENTS.md` から参照させる。口調を変えるときは
コマンド1発で済むようにする。
...
4. **未解決論点3**: `bin/` に口調編集コマンドを新設する。計画書 設計4 の司令官判断どおり、
