...
| 孫 | ブランチ | 内容 | 状況 |
|---|---|---|---|
| 1 | `ocw-pane-roles-01-no-commander` | `ocw -H` に commander 省略モードを追加（`bin/ocw` 本体・`bin/tests/test_ocw.py` に Herdr シムを新設・`bin/README.md`） | ✅ PR #68 マージ済 |
| 2 | `ocw-pane-roles-02-skill-docs` | `umbrella-orchestrator` SKILL.md の更新（孫 spawn を2ペインモードへ・ワークスペースラベルの日本語化・`ai/` 接頭辞前提の除去） | ✅ PR #69 マージ済 |
