...
| サンドボックス実行 | `HOME` と `XDG_DATA_HOME`（配布実体の置き場所。全ツール共通）に加えて、該当する場合は `XDG_CONFIG_HOME` / `XDG_CACHE_HOME` / `ANTIDOTE_HOME` / `CODEX_HOME` を一時ディレクトリに差し替えて実際に deploy | 副作用が `$HOME` 配下に閉じるツール（`bin` `claude` `skills`） | ローカル（CI導入後は CI でも実行） |
