...
「テストを足すかどうか・どこに書くか」の判断基準は `AGENTS.md`「テスト方針」節を参照してください。
役割が違うので併読してください。
...
- すべての deploy スクリプト（`deploy-all.sh` `uninstall.sh` `*/deploy.sh`）は `$HOME` 環境変数を
  ...
  - `skills/deploy.sh`: `CODEX_HOME`（既定 `$HOME/.codex`）/ `XDG_CONFIG_HOME`。配布先
    エージェントのスキルディレクトリの算出に使われる（`shared/helpers.sh` の
    `skill_agent_home`）。`CODEX_HOME` を export している環境でこれを差し替えないと、
    deploy が実ユーザーの Codex 設定ディレクトリへ symlink を張り、uninstall がそれを
    撤去する。**`HOME` の差し替えでは防げない経路**であることに注意する
- 副作用の性質はツールごとに異なる。サンドボックス実行の対象を決める際は、**ネットワークの要否**と
  **副作用が `$HOME` 配下に閉じるか**の2軸で分類する。この分類は配布方式が世代ディレクトリ +
  ...
  - `claude/deploy.sh`: symlink ではなく設定ファイルをマージして実ファイルとして生成する。
    ネットワーク不要、副作用は `$HOME` 配下に閉じる
  - `skills/deploy.sh`: スキルを個別に symlink（`current` 経由）で張る。配布先は Claude Code
    だけでなく Codex・OpenCode のスキルディレクトリも含み（ADR DOC-2608272128）、
    バックアップ先も他ツールと異なる（`<エージェントのホーム>/skills-backup/<名前>.<日時>.<PID>`）。
    ネットワーク不要、副作用は `$HOME` 配下に閉じる
  - `codex/deploy.sh` / `opencode/deploy.sh`: `claude/CLAUDE.md` を単一ソースとして
    symlink するのみ（ADR DOC-2609072334）。エージェントのホームディレクトリが実在する
    場合のみ配布し、存在しないマシンでは何も作成しない（`skills/deploy.sh` と同じ判断
    基準）。ネットワーク不要、副作用は `$HOME` 配下に閉じる
...
| サンドボックス実行 | `HOME` と `XDG_DATA_HOME`（配布実体の置き場所。全ツール共通）に加えて、該当する場合は `XDG_CONFIG_HOME` / `XDG_CACHE_HOME` / `ANTIDOTE_HOME` / `CODEX_HOME` を一時ディレクトリに差し替えて実際に deploy | 副作用が `$HOME` 配下に閉じるツール（`bin` `claude` `skills` `codex` `opencode`） | ローカル + CI（`deploy-smoke` ジョブ。毎PR自動実行） |
| 実マシン検証 | 実 `$HOME` に対する `--dry-run` → 人間の確認 → 人間による実行 | 全ツール（特に `tmux` のパッケージインストール、`zsh` `nvim` のネットワーク処理） | リリース前に人間が行う |

構文検査とドライランはコストが低く副作用も無いため毎コミットで行う。`shellcheck` / `shfmt` は
`.pre-commit-config.yaml` に、CI は `.github/workflows/ci.yml` に導入済みで、`AGENTS.md`
「コミット前の必須ステップ」のとおり毎コミット・毎PRで実行される。サンドボックス実行も
`tests/deploy_smoke.sh` として実装済みで、CI（`.github/workflows/ci.yml` の `deploy-smoke`
ジョブ）が毎PR・毎push(master)で自動実行する。人間もローカルで随時実行できる。

サンドボックス実行は、副作用がすべて一時 `$HOME` 配下で完結するツール
（`bin` `claude` `skills` `codex` `opencode`）に限定する。
...
- [ ] `skills/` のバックアップは `symlink_backup` を通らず、
      `<エージェントのホーム>/skills-backup/<名前>.<日時>.<PID>` に退避されるか（他ツールと異なる例外）
- [ ] スキルが導入済みの全エージェントへ配られ、設定ディレクトリが存在しないエージェントは
      スキップされるか（ADR DOC-2608272128 §2.3）
...
上記チェックリストは `tests/deploy_smoke.sh` として実装済みで、`AGENTS.md`
「コミット前の必須ステップ」がデプロイ関連のシェルスクリプト変更時の実行を必須と定めている。

## AI が守るべきこと

**AI はこの検証（特にサンドボックス実行と実マシン検証）を人間の実 `$HOME` に対して実行してはならない。**
サンドボックス実行の前には、`HOME` に加えて対象ツールが参照する `XDG_CONFIG_HOME` / `XDG_DATA_HOME` /
`XDG_CACHE_HOME` / `ANTIDOTE_HOME` / `CODEX_HOME` が一時ディレクトリを指しているか（または unset
されているか）を必ず確認すること。一覧の実体は `tests/deploy_smoke.sh` の `sandbox_env()` にあり、
**配布先の解決に使う環境変数を増やしたときは、そこにも必ず追加する。**実マシンへの適用が必要な場面では、`--dry-run` の結果を人間に提示し、
実行そのものは人間に委ねる。
