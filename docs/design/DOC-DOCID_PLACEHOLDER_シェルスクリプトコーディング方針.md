...
**shellcheck / shfmt はこのリポジトリにまだ導入されていません（`.shellcheckrc` も導入手順も無く、
導入は別タスクの担当です）。** 導入されるまでは、この文書が「linter に委ねる」としている機械的に
検出できる範囲についても、書き手が自分で意識して確認する必要があります。
...
`echo` より `printf` を優先する（既存コードはほぼ一貫して `printf` を使っている。`nvim/deploy.sh`
に区切り用の `echo ""` が1箇所残っているが、これは例外であり踏襲しない。エスケープシーケンスや
末尾改行の扱いがシェル間で一貫しないため、新規コードは `printf` を使う）。
...
- `deploy-all.sh` `uninstall.sh` を含む全 deploy スクリプトの動作確認は、実 `$HOME` に対して行わない。
  `--dry-run` はすべてのツールに使ってよいが、`HOME` を一時ディレクトリに差し替えるサンドボックス実行が
  許されるのは、副作用が `$HOME` 配下に閉じるツール（`bin` `claude`）に限る。`tmux`（システムへの
  パッケージインストールを伴う）・`zsh` `nvim`（ネットワーク経由の git clone を伴う）は `--dry-run`
  までとし、実際の deploy は人間が実マシンで確認する。対象の分類は「テスト方針」を参照。
