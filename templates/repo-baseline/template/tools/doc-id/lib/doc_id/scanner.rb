...
module DocId
  ...
  module Scanner
    ...
    # 説明的ファイル名部分は、除外文字のブラックリストではなく、実際に使われている
    # 文字種のアローリスト（漢字・ひらがな・カタカナ・長音記号・英数字・ハイフン）で
    # 判定する。ブラックリスト方式だと、句読点・括弧（全角括弧・鉤括弧・句読点など）を
    # 挙げ漏れるたびに、空白を挟まない地の文で「拡張子なしの DOC-ID 言及」の直後に
    # 現れる別の `.md` 言及まで1トークンとして誤って呑み込んでしまう
    # （例:「DOC-XXX_名前）とAGENTS.md」）。
    NAMED_REF = /\b(DOC-\d{6}\d{4}(?:-[a-z])?)_([\p{Han}\p{Hiragana}\p{Katakana}A-Za-z0-9ー_-]+?\.md)(?![a-zA-Z0-9_])/
