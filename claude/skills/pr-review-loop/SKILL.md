...
必要なら実物実行、設定された lint / test の full suite。初回レビューの品質は落とさない。
...
- **現HEADに対する信頼できる full-suite 成功結果がすでに存在するなら、レビュワー自身が
  full suite を再実行する必要はない。** 信頼できる結果の例:
  - 実装者が現HEADで実行し、GitHubコメントに結果を記録している
  - pre-commit フックが現HEADのコミット作成時に full suite を実行している
  - CI が現HEADで green
- ただし**その結果が本当に現HEADに対応しているか**（記録されたSHAと現HEADの一致）を必ず確認する
...
2. 全返信後、PRレベルのサマリーコメントで全指摘の対応状況を個別に列挙し、テスト結果も記載する。
   ...
   - 修正が**コメント・docs のみ**（runtime behavior に影響しない）の場合は、その旨を明記する。
     レビュワーは runtime の full suite を実行せず、差分に直接関係する検証だけを行う。
3. 新しいHEAD SHAを取得:
...
4. **最小限の**再レビュー依頼を送信。修正内容を列挙するな。レビュワーはGitHubのコメントを読む。
   ただし**差分の起点となる前回レビュー対象SHA（このラウンドの `$HEAD_SHA`）は必ず渡す**。
   レビュワーはこの区間の差分を分類してから検証範囲を決めるため、起点が無いと full suite を
   回し直すことになる:

```bash
herdr pane run "$REVIEWER_PANE" "PR #$PR 再レビュー依頼。全指摘に対応コメント書きました。前回レビュー対象: $HEAD_SHA → 現HEAD: $NEW_HEAD_SHA"
```
...
3. **現HEADに対する full-suite 保証が存在することを確認。** 承認条件は「レビュワー自身が
   full suite を回した」ではなく、**「現HEADに対する信頼できる full-suite 成功結果
   （実装者が現HEADで実行してGitHubコメントに記録した結果 / pre-commit フックによる実行 /
   CI green のいずれか）が存在し、レビュワーがそれを確認した」**である。現HEADに対する結果が
   確認できない場合は完了報告に進まず、`TEST_CMD` を実行して結果を現HEADのSHA付きで記録する。
4. CIがパスしていることを確認。
