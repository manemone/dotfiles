...
`{{RISK_BASED_TESTING}}` だけは値ではなく、**本スキルの「テストの判断基準（risk-based testing）」
節の本文**で置換する。レビュワーはこのファイルしか読まないため、節への参照ではなく本文を貼ること
（実装者とレビュワーが同じ基準を使うことが目的なので、片方だけ書き換えない）。
見出し行 `## テストの判断基準（risk-based testing）` と、その直下の「実装者とレビュワーが
同じ基準で使う」の1行（スキル内部の案内であり、レビュワーには不要）は貼らない。
...
唯一の例外は**テストを追加するかどうかの判断だけ**で、これは「テスト不足の判定基準」の入口判定を通す。実バグ・安全性・データ破壊・後方互換性を含むそれ以外の観点は、一切伏せない。**バグそのものを「テストの話だから」と黙るのは、この例外の対象外である。**

**3. finding の入口と、finding 後の扱いは別。**
まず「そもそもこれは finding か」を判断する。特にテストについては、**「追加するとより安心」だけでは finding ではない**（「テスト不足の判定基準」を通すこと）。
