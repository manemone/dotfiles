...
| `pr-group-review` | 他人の PR 群（複数リポにまたがるものを含む。1 本だけでもよい）をレビュワーとしてレビューする。読み取り専用で取得し、PR 群の地図と継ぎ目を見て、人間の確認後に COMMENT だけを投稿する。修正コードは書かない。地図テンプレート・継ぎ目チェックリスト・状態ファイル形式を `references/` に持つ |
| `pr-group-request` | 自分の PR 群（傘ブランチの計画書からも作れる）のレビューを他人に頼む準備をする。地図と継ぎ目の自己チェック、人間の確認後の PR 本文の更新、チャット用の依頼文の下書きを用意する（投稿は人間）。参照文書は `pr-group-review/references/` を相対パスで再利用する |
