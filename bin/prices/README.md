...
（イベントを書き換えずに）読み取り時に検出し、footer の `price_table:` 行の件数と、
`report --month` の `price_tables_applied` に「対象月より後に発効」として警告する。

## 時間帯（peak/off-peak）課金の書き方
...
- **境界は開始時刻を含み、終了時刻を含まない**（`start <= t < end`）。
  ...
  境界ちょうどのタイムスタンプがどちらか片方だけに一意に決まる
...
- **窓の `models` に載っていないモデルは、その窓の時間帯であっても常に基本の `models`
  ...
  一部のモデルだけ時間帯課金にできる
- 書き込まれたイベントの `time_of_day_basis` フィールド
  （`not_applicable` / `peak` / `off_peak` / `unknown_timestamp`）で、実際にどの単価が
  適用されたかを後から確認できる
